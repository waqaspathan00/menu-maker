        ...
        <header className="w-full  h-auto shadow p-4">
                    ...
                    <div className="flex items-center gap-x-4 relative">
                        ...
                        <div className="xl:contents lg:contents hidden">
                            ...
                            <Menu as="div" className="relative">
                                <Menu.Button
                                    className="flex items-center gap-x-2">Learn <BsFillCaretDownFill/></Menu.Button>
                                <Transition
                                    as={React.Fragment}
                                ...
                                >
                                    <Menu.Items as="ul"
                                                className="absolute top-8 left-0 w-fit whitespace-nowrap border bg-white border-primary-gray/40 shadow-md rounded text-sm">
                                        <Menu.Item as="li"
                                                   className="hover:bg-primary-blue hover:text-white transition-colors px-4 py-2">
            ...
            >
                <div className="grid gap-2 h-auto absolute top-21 bg-white w-full shadow-xl left-0 p-4 border z-50">
                    ...
                    <Link href="#">About Us</Link>
                    {!userData.user && !userData.loading ? <button
                        className="px-8 border py-2 rounded-full  border-primary-blue  text-primary-blue hover:bg-primary-blue hover:text-white transition-colors flex items-center justify-center xl:w-fit lg:w-fit md:w-fit w-full"
                        onClick={signInWithGoogle}><BsGoogle className="mr-2"/> Sign in with Google </button> : null}
                </div>
                {!userData.loading && userData.user ? null :
                    <div className="pt-2">
                        <button
                            className="px-8 border py-2 rounded-full  border-primary-blue  text-primary-blue hover:bg-primary-blue hover:text-white transition-colors flex items-center w-full justify-center"
                            onClick={signInWithGoogle}><BsGoogle className="mr-2"/> Sign in with Google
                        </button>
                    </div>}
            </Transition>
        </header>
