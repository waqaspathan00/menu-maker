    ...
    return (
        <div className='fixed w-screen h-screen bg-primary-black/20 top-0 left-0 z-50 grid place-items-center'>
            <div className='w-auto h-auto bg-white p-8 space-y-2 rounded relative'>
                <IoCloseOutline className="absolute cursor-pointer right-2 top-3 w-6 h-6"
                                onClick={() => setIsOpen(false)}/>
                <div id="qr-code" className='w-48 mx-auto'/>
                <div className='mt-4 w-fit mx-auto'>
                    <button
                        className='cursor-pointer text-base border p-2 rounded hover:bg-primary-blue hover:text-white transition-colors'
                        onClick={handleDownload}>Download QR Code
                    </button>
                </div>
            </div>
        </div>
